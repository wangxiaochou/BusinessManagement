...
import com.xzty.cq.tover.businessmanagement.projectmanagement.tool.presenter.projectmanage.affirm.AffirmDetailsContract;
import com.xzty.cq.tover.businessmanagement.projectmanagement.tool.presenter.projectmanage.affirm.AffirmDetailsPresenter;
...
public class ToolPMApplyAffirmDetailsActivity extends ActivityPresenter<AffirmDetailsContract.Presenter> implements AffirmDetailsContract.View, BaseQuickAdapter.OnItemLongClickListener, BaseQuickAdapter.OnItemClickListener {
    ...
    protected int getContentLayoutId() {
        return R.layout.activity_tool_apply_affirm_details;
    }
    ...
    public void detailsSuccess(List<RspAffirmDetails> list) {
        mList = list;
        setAdapter();
        tmdacdTvSum.setText(("共计 " + mList.size() + "项：拒绝 0" + "，移交采购部：0" + "，待发货：0"));
    }

    @Override
    public void reFreshAdapter(List<RspAffirmDetails> sureList, int check, int refuse) {
        mList = sureList;
        mAdapter.notifyDataSetChanged();
        tmdacdTvSum.setText(("共计" + mList.size() + "项：拒绝" + refuse + "，移交采购部：" + (mList.size() - check - refuse) + "，待发货：" + check));
    }
    ...
    @Override
    protected AffirmDetailsContract.Presenter initPresenter() {
        return new AffirmDetailsPresenter(this);
    }
