...
public class PDAffirmOrderHelper {
    public static void getDetails(int id, final DataSourse.Callback callback) {
        RemoteService service = NetWork.remote(RemoteService.class);
        Subscription subscription = service.getToolBDApplyConfirmDetail(id).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(new Subscriber<RspModel<List<RspAffirmDetails>>>() {
            @Override
    ...
    private static void commit(List<RspAffirmDetails> sureList, int applyId, final DataSourse.Callback callback) {
        ...
        RemoteService service = NetWork.remote(RemoteService.class);
        Subscription subscription = service.bdSubmitApplyConfirm(commit).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(new Subscriber<RspModel<List<RspLogin>>>() {
            @Override
            public void onCompleted() {
    ...
    public static LongClickModel longClick(List<RspAffirmDetails> sureList, int position) {
        LongClickModel model = new LongClickModel();
        if (sureList.get(position).isRefuse) {
            sureList.get(position).isRefuse = false;
        } else {
            sureList.get(position).isRefuse = true;
        }
        int check = 0, refuse = 0, ignore = 0;
        for (RspAffirmDetails detail : sureList) {
            ...
            if (detail.getToolApplyDetailState() != 2) {
                ignore++;
            }
        }
    ...
    public static LongClickModel itemClick(List<RspAffirmDetails> sureList, int position) {
        LongClickModel model = new LongClickModel();
